...
public class ViewBookingsGUIView extends PageGUIView {
    ...
    public void buildContent(BorderPane root,
        ...
        Runnable refreshList = () -> {
            ...
            if (btnConfirmed.isSelected()) {
                if (confirmed.isEmpty()) listBox.getChildren().add(emptyLabel("Non hai prenotazioni confermate."));
                else for (BookingResponseBean b : confirmed) listBox.getChildren().add(buildBookingCard(b, true, studentId, onCancel));
            } else if (btnCancelled.isSelected()) {
                if (cancelled.isEmpty()) listBox.getChildren().add(emptyLabel("Non hai prenotazioni cancellate."));
                else for (BookingResponseBean b : cancelled) listBox.getChildren().add(buildBookingCard(b, false, studentId, onCancel));
            } else {
    ...
    private void refreshPastList(VBox listBox, List<BookingResponseBean> past, String subjectFilter) {
        ...
        if (filtered.isEmpty()) { listBox.getChildren().add(emptyLabel("Nessuna lezione scaduta.")); return; }

        List<String> subjects = filtered.stream().map(b -> b.getSubject().getName()).distinct().sorted().toList();
        ...
        for (String subject : subjects) {
            ...
            Label countLabel = new Label("(" + group.size() + " " + (group.size() == 1 ? "lezione" : "lezioni") + ")");
            ...
            for (int i = 0; i < group.size(); i++) {
                ...
                dateLbl.getStyleClass().add("register-label"); dateLbl.setPrefWidth(90);
                Label timeLbl = new Label(b.getTimeSlot().getStartTime() + " – " + b.getTimeSlot().getEndTime());
                timeLbl.getStyleClass().add("info-text"); timeLbl.setPrefWidth(110);
                ...
                if (b.getMeetLink() != null && !b.getMeetLink().isBlank()) {
                    ...
                    meet.setOnAction(e -> { try { java.awt.Desktop.getDesktop().browse(new java.net.URI(b.getMeetLink())); } catch (Exception ex) { /* ignora */ } });
                    row.getChildren().add(meet);
                }
                groupCard.getChildren().add(row);
                if (i < group.size() - 1) {
                    Region line = new Region(); line.setPrefHeight(1); line.setMaxWidth(Double.MAX_VALUE);
                    line.setStyle("-fx-background-color: #eef2f6;"); line.setMouseTransparent(true);
    ...
    private FlowPane buildSubjectFilterBar(List<String> subjects) {
        ...
        bar.setAlignment(Pos.CENTER_LEFT); bar.setMaxWidth(640); bar.setPadding(new Insets(0, 0, 4, 0));
        return bar;
    }
    ...
    private VBox buildBookingCard(BookingResponseBean b, boolean cancellable, int studentId, BiConsumer<BookingResponseBean, Integer> onCancel) {
        ...
        DateTimeFormatter fmt = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        Label dateTime = new Label(b.getTimeSlot().getDate().format(fmt) + "   " + b.getTimeSlot().getStartTime() + " – " + b.getTimeSlot().getEndTime());
        dateTime.getStyleClass().add("welcome-label");
        HBox dateRow = new HBox(8, dot, dateTime); dateRow.setAlignment(Pos.CENTER_LEFT);
        ...
        Label subject = new Label("Materia: " + b.getSubject().getName()); subject.getStyleClass().add("small-label");
        Label tutor = new Label("Tutor: " + b.getTutor().getName() + " " + b.getTutor().getSurname()); tutor.getStyleClass().add("register-label");
        info.getChildren().addAll(dateRow, status, subject, tutor);
        if (b.getTutor().getEmail() != null) { Label te = new Label("Email  " + b.getTutor().getEmail()); te.getStyleClass().add("info-text"); info.getChildren().add(te); }
        if (cancellable || (b.getMeetLink() != null && !b.getMeetLink().isBlank())) {
            HBox bottomRow = new HBox(); bottomRow.setAlignment(Pos.CENTER_LEFT); bottomRow.setMaxWidth(Double.MAX_VALUE);
            if (b.getMeetLink() != null && !b.getMeetLink().isBlank()) {
                Hyperlink meet = new Hyperlink("🎥  Apri Meet"); meet.getStyleClass().add("hyperlink");
                meet.setOnAction(e -> { try { java.awt.Desktop.getDesktop().browse(new java.net.URI(b.getMeetLink())); } catch (Exception ex) { /* link non apribile */ } });
                bottomRow.getChildren().add(meet);
            }
            Region spacer = new Region(); HBox.setHgrow(spacer, Priority.ALWAYS); bottomRow.getChildren().add(spacer);
            if (cancellable) { Button cancelBtn = new Button("Annulla"); cancelBtn.getStyleClass().add("danger-button"); cancelBtn.setOnAction(e -> onCancel.accept(b, studentId)); bottomRow.getChildren().add(cancelBtn); }
