...
public class TutorDAODB implements TutorDAO {
    ...
    public List<Tutor> getBySubject(Subject subject) throws DAOException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) result.add(mapToTutor(rs));
            }
    ...
    public Tutor findById(int id) throws DAOException {
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return mapToTutor(rs);
            }
        } catch (SQLException e) {
    ...
    private Tutor mapToTutor(ResultSet rs) throws SQLException {
        return new Tutor(rs.getInt("id"), rs.getString("name"),
                rs.getString("surname"), rs.getString("email"),
                null, rs.getString("bio"));
    }
}